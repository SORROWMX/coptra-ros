...
@pytest.fixture()
def node():
    return rospy.init_node('coptra_test', anonymous=True)
...
def test_blocks(node):
    rospy.wait_for_service('coptra_blocks/run', timeout=5)
    rospy.wait_for_service('coptra_blocks/stop', timeout=5)
    rospy.wait_for_service('coptra_blocks/load', timeout=5)
    rospy.wait_for_service('coptra_blocks/store', timeout=5)

    from std_msgs.msg import String
    from coptra_blocks.srv import Run

    def wait_print():
        try:
            wait_print.result = rospy.wait_for_message('coptra_blocks/print', String, timeout=5).data
    ...
    run = rospy.ServiceProxy('coptra_blocks/run', Run)
...
def test_long_callback():
    from coptra import long_callback
